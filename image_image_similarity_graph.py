...
parser.add_argument('--patient_type', type=str, default="multiple")
...
for batch_idx, batch in enumerate(test_dataloader_selected):
    ...
    with torch.no_grad():
        ...
        for p in range(attr_classes_bypatient.shape[0]):
            ...
            #                             patient_ind=count_correct1, patient_disease=patient_disease,preds=pred_disease)
            ...
            #     pred_disease=[exist_labels[i] for i, label in enumerate(Y_hat) if label==1]
            #     plot_similarity_ingraph(torch.Tensor(patients_maps), classes_map_bypatient[p], image[p], compute_similarity,  exist_labels=exist_labels,
            #                             patient_ind=count_correct2, patient_disease=patient_disease,preds=pred_disease)
            #     saving_graph_path="/home/shelley/Documents/diffbygraph/graph_visualisation_gradcamnew/seed{}/correct_preded_single/".format(str(args.seed))+"{}_img_{}_lr{}_epoches{}/{}/".format(args.name, args.interp, args.lr, args.epochs,patient_disease[0])
            #     os.makedirs(saving_graph_path,exist_ok=True)
            ...
            #     pred_disease=[exist_labels[i] for i, label in enumerate(Y_hat) if label==1]
            #     plot_similarity_ingraph(torch.Tensor(patients_maps), classes_map_bypatient[p], image[p], compute_similarity,  exist_labels=exist_labels,
            #                             patient_ind=count_correct3, patient_disease=patient_disease,preds=pred_disease)
            #     saving_graph_path="/home/shelley/Documents/diffbygraph/graph_visualisation_gradcamnew/seed{}/correct_preded_single/".format(str(args.seed))+"{}_img_{}_lr{}_epoches{}/{}/".format(args.name, args.interp, args.lr, args.epochs,patient_disease[0])
            #     os.makedirs(saving_graph_path,exist_ok=True)
            ...
            #     pred_disease=[exist_labels[i] for i, label in enumerate(Y_hat) if label==1]
            #     plot_similarity_ingraph(torch.Tensor(patients_maps), classes_map_bypatient[p], image[p], compute_similarity,  exist_labels=exist_labels,
            #                             patient_ind=count_correct4, patient_disease=patient_disease,preds=pred_disease)
            #     saving_graph_path="/home/shelley/Documents/diffbygraph/graph_visualisation_gradcamnew/seed{}/correct_preded_single/".format(str(args.seed))+"{}_img_{}_lr{}_epoches{}/{}/".format(args.name, args.interp, args.lr, args.epochs,patient_disease[0])
            #     os.makedirs(saving_graph_path,exist_ok=True)
            ...
            #     pred_disease=[exist_labels[i] for i, label in enumerate(Y_hat) if label==1]
            #     plot_similarity_ingraph(torch.Tensor(patients_maps), classes_map_bypatient[p], image[p], compute_similarity,  exist_labels=exist_labels,
            #                             patient_ind=count_correct5, patient_disease=patient_disease,preds=pred_disease)
            #     saving_graph_path="/home/shelley/Documents/diffbygraph/graph_visualisation_gradcamnew/seed{}/correct_preded_single/".format(str(args.seed))+"{}_img_{}_lr{}_epoches{}/{}/".format(args.name, args.interp, args.lr, args.epochs,patient_disease[0])
            #     os.makedirs(saving_graph_path,exist_ok=True)
            ...
            if (torch.eq(Y_hat,patient_label).all().cpu().numpy()) and count_wrong<20:
                pred_disease=[exist_labels[i] for i, label in enumerate(Y_hat) if label==1]
                plot_similarity_ingraph(torch.Tensor(patients_maps), classes_map_bypatient[p], image[p], compute_similarity,  exist_labels=exist_labels,
                                        patient_ind=count_correct, patient_disease=patient_disease,preds=pred_disease)
                ...
                pred_disease=[exist_labels[i] for i, label in enumerate(Y_hat) if label==1]
                plot_similarity_ingraph(torch.Tensor(patients_maps), classes_map_bypatient[p], image[p], compute_similarity,  exist_labels=exist_labels,
                                        patient_ind=count_wrong, patient_disease=patient_disease,preds=pred_disease)
